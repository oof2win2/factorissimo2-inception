...
local function create_interface(player)
	...
	if screen["f2pl_input_selector"] ~= nil then
		...
		local top_buttons_flow = top_content.add { type = "flow", name = "top_buttons_flow", direction = "horizontal", style =
		"f2pl_horizontal_centerflow" }
		...
		local bottom_buttons_flow = bot_content.add { type = "flow", name = "bottom_buttons_flow", direction =
		"horizontal", style = "f2pl_horizontal_centerflow" }

		local entity = game.get_surface(global.factory.placed_on_surface_name).find_entity(global.factory.name,
			global.factory.position)
...
local function set_item_input(direction, id, input)
	...
	local inBeltE = indoorSurface.create_entity { name = belt_tier_name .. "transport-belt", position =
		indoorBeltPosition, direction = entityDirection, force = "player", raise_built = true }
...
local function set_fluid_input(direction, id, input)
	...
	local infinitypipe = outdoorSurface.create_entity { name = "infinity-pipe", position = infPipePosition, force =
	"player" }
	local outPipeE = outdoorSurface.create_entity { name = "factory-input-pipe", position = outdoorBulkpipePosition, force =
	"player", type = "output", direction = entityDirection, raise_built = true }
	local inPipeE = indoorSurface.create_entity { name = "factory-output-pipe", position = indoorBulkpipePosition, direction =
		entityDirection, force = "player", raise_built = true }
		...
		if entity.name == "infinity-pipe" then
			...
			if filter ~= nil then
				if global.availableItems[filter.name .. "-barrel"] ~= nil then
					global.availableItems[filter.name .. "-barrel"].amount = global.availableItems
						[filter.name .. "-barrel"].amount + 1
				else
...
guis.on_init = function()
	...
	-- the global belt tier name will be the slowest belt
	local slowest_belt = nil
	for _, prototype in pairs(game.entity_prototypes) do
		if prototype.type == "transport-belt" then
			if slowest_belt == nil then
				slowest_belt = prototype.name
			else
				if prototype.belt_speed < game.entity_prototypes[slowest_belt].belt_speed then
					slowest_belt = prototype.name
				end
			end
		end
